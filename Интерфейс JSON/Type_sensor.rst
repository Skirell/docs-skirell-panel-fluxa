...
param_1
*******
   Используется для изменения текста названия в UI-блоке, например, чтобы указать, что это датчик «Температура» или «Влажность». Может применяться и в других целях.
...
measure
*******
   Используется для изменения текста единицы измерения в UI-блоке, например, чтобы указать, что это единица измерения «°C» или «%». Может применяться и в других целях.
...
min
*******
   Задаёт минимальное значение шкалы кругового индикатора в UI-блоке.
...
stage_1
*******
   Задаёт первую пороговую границу шкалы кругового индикатора в UI-блоке. Используется для определения цветового состояния кругового индикатора при сравнении с текущим значением датчика.
...
stage_2
*******
   Задаёт вторую пороговую границу шкалы кругового индикатора в UI-блоке. Используется для определения цветового состояния кругового индикатора при сравнении с текущим значением датчика.
...
max
*******
   Задаёт максимальное значение шкалы кругового индикатора в UI-блоке.
...
color_2
*******
   Устанавливает указанный цвет кругового индикатора при при значении в диапазоне от ``stage_1`` до ``stage_2`` включительно. Аналогичный вариант описания ``(stage_1 >= значение датчика <= stage_2) == color_2``.
