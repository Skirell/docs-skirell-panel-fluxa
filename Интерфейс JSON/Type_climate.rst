...
min_target
****
   Устанавливает минимальное допустимое значение уставки, которое можно выбрать с помощью кругового слайдера. Пользователь не сможет установить значение меньше этого порога.
...
max_target
****
   Устанавливает максимальное допустимое значение уставки, которое можно выбрать с помощью кругового слайдера. Пользователь не сможет установить значение больше этого порога.
...
measure
*******
   Устанавливает единицу измерения заданного значения (уставки), например, чтобы указать, что это единица измерения «°C» или «%». Может применяться и в других целях.
...
fan_command_topic
*******************
   Командный MQTT-топик, куда отправляется сообщение с установленным пользователем режимом скорости.
...
fan_state_topic
*******************
   MQTT-топик обратной связи, откуда приходит сообщение с установленным устройством режимом скорости.
...
modes
*******************
   Массив с обозначениями режимов, которые соответствуют состоянию включенного устройства. Режим "Выкл" установлен автоматически, убрать его нельзя, можно изменить только команду ``payload``, который он отправляет/принимает. Массив предполагает настройку от 1-ого до 4-х режимов (не включая режим "выкл"), а именно иконки ``icon``, названия режима ``title``, отображаемого в настройках, цвет ``color`` кругового ползунка при выборе соответствующего режима, который так же выбирается из приведенной таблицы выше, и сообщения ``payload``, которое режим отправялет/слушает в MQTT-топиках.
...
fan_modes
*******************
  Массив с обозначениями дополнительных режимов. Массив предполагает настройку от 0 до 5 режимов, а именно иконки ``icon`` и сообщения ``payload``, которое режим отправялет/слушает в MQTT-топиках.
...
Пример для климатического устройства без дополнитлеьных режимов::
