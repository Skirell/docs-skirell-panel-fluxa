...
*******
   Используется как цвет для активного состояния устройства (при режиме ВКЛ). Имеет ряд особенностей:
      * В подтипе ``climate_variant_thermostat`` данный параметр отвечает за цвет блока в интерфейсе, а также за цвет ползунка внутри страницы настроек.
      * В подтипе ``climate_variant_cond`` данный параметр отвечает только за цвет блока в интерфейсе. Цвета ползунков внутри страницы настроек задаются другими параметрами, которые описаны в разделе с подробным описанием подтипа.
...
************
   ...
   * ограничение строки: Только заданные значения
   * **варианты:**
      * :ref:`climate_variant_thermostat` — устройство с термостатом.
      * :ref:`climate_variant_cond` — устройство с кондиционированием.
...
ПЕРЕДЕЛАТЬ Отправляет сообщение с режимом на командный MQTT-топик ``mode_command_topic``, а также отслеживает состояние устройства через MQTT-топик обратной связи ``mode_state_topic`` для обновления элементов интерфейса.

ВСТАВИТЬ КАРТИНКУ

Параметры ``variant``:

* :ref:`mode_command_topic`: 
* :ref:`mode_state_topic`: 
* :ref:`off_payload`: 
* :ref:`modes`: Массив режимов
* :ref:`currentTemp_state_topic`: 
* :ref:`targetTemp_command_topic`: 
* :ref:`targetTemp_state_topic`: 
* :ref:`fan_command_topic`: 
* :ref:`fan_state_topic`: 
* :ref:`fan_modes`: Массив скоростей
