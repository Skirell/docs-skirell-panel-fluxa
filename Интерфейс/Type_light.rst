...
param_2
*******
   Используется для изменения текста в UI-блоке. Например, позволяет уточнить, где будет находится данное устройство-освещение. Если панель установлена в конкретной комнате, а все освещение относится к ней, это поле можно оставить пустым и написать значение из ``param_1`` сюда.
...
variant_type
************
   Используется для указания названия подварианта (подтипа) устройства-освещения.
...
Отправляет указанную команду включения или выключения (``payload_on`` / ``payload_off``) на командный MQTT-топик ``OnOff_command_topic``, а также отслеживает состояние устройства через MQTT-топик обратной связи ``OnOff_state_topic`` для обновления элементов интерфейса.
