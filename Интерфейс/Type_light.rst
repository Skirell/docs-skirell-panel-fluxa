...
Блок ``light`` предназначен для управления освещением и поддерживает четыре типа устройства. При кратковременном нажатии на блок отправляется команда включения или выключения (``payload_on`` / ``payload_off``). При длительном удержании открывается страница настроек блока (за исключением типа ``light_variant_OnOff``).
Общие параметры:
